...
void PrintStatTables(vector<OneThreadTest>& testers, const size_t nTests) {
    ...
    for (size_t i = 0; i < 256; ++i) {
        ...
        for (size_t j = 0; j < 256; ++j) {
            cout << setprecision(6) << fixed << elements[j] << " ";
        }
        cout << endl;
    }
}
...
int main(int argc, const char* argv[]) {
    if (argc < 2) {
        cerr << "usage: " << argv[0] << " <threads count>" << endl;
        return 1;
    }
    size_t sz;
    const size_t WorkersCount = stoll(argv[1], &sz);
    const size_t NTests = 10000 * 256 * 256;
    assert(sz == string(argv[1]).size());

    vector<OneThreadTest> testers;
    vector<thread> workers;
    for (size_t i = 0; i < WorkersCount; ++i)
        testers.emplace_back(OneThreadTest(i, NTests / WorkersCount));
    for (size_t i = 0; i < WorkersCount; ++i)
